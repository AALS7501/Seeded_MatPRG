...
For this circuit, `M1` is the witnessed seed length in bits, `M2 × K` is the size of the expanded binary key matrix used by the outer MatPRG step, and `N1 × Low_bits` is the number of bits produced by the truncation step before parsing. In the paper-facing presets, we use an 80-bit generic-attack calibration for the binary-input instances: `M1` is chosen to satisfy both `M1 > N1` and `M1 >= 160`, while `M2` is chosen so that the outer meet-in-the-middle/dissection proxy `M2/2 - log2(K)` stays at or above 80 bits across the reported range. This is a generic binary-input calibration for the instances used in the proofs, not a full lattice-hardness estimate.
...
| 6 | 64 KB | 2048 | 79 | 160 | 170 | 26 | 36 | 123 |
| 7 | 128 KB | 4096 | 108 | 160 | 171 | 38 | 53 | 123 |
| 8 | 256 KB | 9182 | 156 | 160 | 172 | 59 | 83 | 123 |
| 9 | 512 KB | 16000 | 174 | 160 | 175 | 92 | 131 | 123 |
| 10 | 1 MB | 32000 | 175 | 270 | 176 | 183 | 269 | 120 |
| 11 | 2 MB | 62000 | 176 | 531 | 177 | 353 | 530 | 118 |
| 12 | 4 MB | 124000 | 180 | 1058 | 181 | 689 | 1057 | 118 |

...
fn matprg_image(circuit: &RegisterMatPRGCircuit<C, GG>) -> Vec<Fr> {
...
fn bench_register_MatPRG(log: usize) {
    ...
    let c0 = RegisterMatPRGCircuit::<C, GG>::generate_circuit_with_matrix(
        rc.clone(),
        matrix_a.clone(),
        &mut rng,
    )
    ...
    eprintln!("  constraints = {n}");
    if constraints_only_mode() {
        println!("register_MatPRG,{log},{n},0.0,0.0,0");
        return;
    }
    ...
    let verify_us = (0..N_ITER)
        ...
        / N_ITER as f64;
    eprintln!("  verify = {verify_us:.0} µs (mean)");
    println!("register_MatPRG,{log},{n},{setup_ms:.1},{prove_ms:.1},{verify_us:.0}");
}

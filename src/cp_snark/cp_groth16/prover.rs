use crate::cp_snark::cp_groth16::{r1cs_to_qap::R1CSToQAP, CPGroth16, Proof, ProvingKey, VerifyingKey};
...
impl<E: Pairing, QAP: R1CSToQAP> CPGroth16<E, QAP> {
    ...
    pub fn create_proof_with_reduction_and_matrices(
        ...
        let prover_time = start_timer!(|| "CPGroth16::Prover");
    ...
    fn create_proof_with_assignment(
        ...
        let committed_size = pk.vk.committed_size as usize;
        let l_aux_acc = E::G1::msm_bigint(&pk.l_query, &aux_assignment[committed_size..]);
        ...
        let cm_time: ark_std::perf_trace::TimerInfo = start_timer!(|| "Compute CM");

        let v_eta_gamma_inv = pk.vk.eta_gamma_inv_g1.into_group() * v;
        let gamma_abc_inputs_acc_without_one = E::G1::msm_bigint(
            &pk.vk.gamma_abc_g1[instance_assignment.len()..],
            &aux_assignment[0..committed_size],
        );

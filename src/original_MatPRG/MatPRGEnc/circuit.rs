...
pub struct MatPRGEncCircuit<C: CurveGroup, GG: CurveVar<C, ConstraintF<C>>>
where
...
#[allow(non_snake_case)]
impl<C, GG> ConstraintSynthesizer<C::BaseField> for MatPRGEncCircuit<C, GG>
where
    C: CurveGroup,
    ...
    for<'a> &'a GG: GroupOpsBounds<'a, C, GG>,
{
    fn generate_constraints(
        ...
        let data_key: Vec<FpVar<<<C as CurveGroup>::Affine as AffineRepr>::BaseField>> =
        ...
        let check_H_ct = MiMCGadget::<C::BaseField>::evaluate(&rc, &CT).unwrap();
        H_ct.enforce_equal(&check_H_ct)?;

        // check h_k
        let check_h_k = MiMCGadget::<C::BaseField>::evaluate(&rc, &data_key.clone()).unwrap();
        h_k.enforce_equal(&check_h_k)?;

        // check A * K = R

        let flag1 = cs.num_constraints();
        ...
        println!("flag1 = {:?}", cs.num_constraints() - flag1);
        ...
        println!("flag2 = {:?}", cs.num_constraints() - flag2);

        let flag3 = cs.num_constraints();
        ...
        println!("flag3 = {:?}", cs.num_constraints() - flag3);
...
#[allow(non_snake_case)]
impl<C, GG> MockingCircuit<C, GG> for MatPRGEncCircuit<C, GG>
where
    C: CurveGroup,
    GG: CurveVar<C, C::BaseField>,
    <C as CurveGroup>::BaseField: PrimeField + Absorb,
    for<'a> &'a GG: GroupOpsBounds<'a, C, GG>,
{
    ...
    type Output = MatPRGEncCircuit<C, GG>;
    ...
    fn generate_circuit<R: ark_std::rand::Rng>(
        ...
        let h_ct = Self::H::evaluate(&rc.clone(), CT.clone()).unwrap();

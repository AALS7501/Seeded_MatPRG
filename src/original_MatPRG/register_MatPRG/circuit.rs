...
impl<C, GG> RegisterMatPRGCircuit<C, GG>
where
    C: CurveGroup,
...
{
    pub fn sample_public_matrix<R: ark_std::rand::Rng>(rng: &mut R) -> Matrix<C::BaseField> {
        let matrix_a_values: Vec<Vec<C::BaseField>> = (0..DATA_SET.N)
            .map(|_| (0..DATA_SET.M).map(|_| C::BaseField::rand(rng)).collect())
            .collect();
        Matrix::new(matrix_a_values)
    }

    pub fn generate_circuit_with_matrix<R: ark_std::rand::Rng>(
        rc: mimc7::Parameters<C::BaseField>,
        matrix_A: Matrix<C::BaseField>,
        rng: &mut R,
    ) -> Result<Self, Error> {
        ...
        let data: Vec<C::BaseField> = (0..DATA_SET.Data_size)
            ...
            .collect();
        let basefield_key: Vec<C::BaseField> =
            (0..DATA_SET.Key_len).map(|_| C::BaseField::rand(rng)).collect();

        let mut key_bit: Vec<bool> = Vec::new();
        for f in &basefield_key {
            let mut bits = f.into_bigint().to_bits_le();
            bits.resize(256, false);
            key_bit.extend_from_slice(&bits);
        }

        let matrix_K_values: Vec<Vec<C::BaseField>> = (0..DATA_SET.M)
            .map(|i| {
                (0..DATA_SET.K)
                    .map(|j| C::BaseField::from(key_bit[i * DATA_SET.K + j]))
                    .collect()
            })
            .collect();
        ...
        let CT: Vec<C::BaseField> = data
            ...
            .enumerate()
            .map(|(i, d)| {
                let ks = if i < DATA_SET.K {
                    matrix_R.matrix[0][i]
                } else {
                    matrix_R.matrix[i / DATA_SET.K][i % DATA_SET.K]
                };
                *d + ks
            })
            .collect();
        ...
        let h_k = mimc7::MiMC::<C::BaseField>::evaluate(&rc, key_with_sk)?;

        let gamma_matrix: Vec<Vec<C::BaseField>> = (0..DATA_SET.K)
            .map(|_| vec![C::BaseField::rand(rng)])
            .collect();
    ...
    for<'a> &'a GG: GroupOpsBounds<'a, C, GG>,
{
    fn generate_constraints(self, cs: ark_relations::r1cs::ConstraintSystemRef<C::BaseField>) -> Result<(), SynthesisError> {
        ...
        let matrix_A = MatrixVar::new_constant(
            cs.clone(),
        ...
        let gamma: MatrixVar<C::BaseField> = MatrixVar::new_input(
            cs.clone(),
    ...
    for<'a> &'a GG: GroupOpsBounds<'a, C, GG>,
{
    ...
    fn generate_circuit<R: ark_std::rand::Rng>(rc: Self::HashParam, rng: &mut R) -> Result<Self, Error> {
        let matrix_A = Self::sample_public_matrix(rng);
        Self::generate_circuit_with_matrix(rc, matrix_A, rng)
    }
}

mod test {
    ...
    use crate::registerdatav1;
    use crate::registerdatav1::circuit::RegisterdataCircuit;
    ...
    fn test_registerdatav1() {
        ...
        let end = Instant::now();
        let res = Groth16::<Bn254>::verify_with_processed_vk(&pvk, &image, &proof).unwrap();
        // assert!(Groth16::<Bn254>::verify_with_processed_vk(&pvk, &image, &proof).unwrap());
        println!("verify time? = {:?}", end.elapsed());
    }
}

// cargo test --release --package zkMarket --features registerdata,trace,parallel -- registerdatav1::tests::test_registerdatav1::test::test_registerdatav1 --exact --show-output
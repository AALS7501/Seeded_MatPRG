...
impl<C, GG> RegisterSeededMatPRGCircuit<C, GG>
where
    C: CurveGroup,
...
{
    pub fn sample_public_matrices<R: ark_std::rand::Rng>(
        rng: &mut R,
    ) -> (Matrix<C::BaseField>, Matrix<C::BaseField>) {
        let matrix_A1_values: Vec<Vec<C::BaseField>> = (0..DATA_SET.N1)
            .map(|_| (0..DATA_SET.M1).map(|_| C::BaseField::rand(rng)).collect())
            .collect();
        let matrix_A2_values: Vec<Vec<C::BaseField>> = (0..DATA_SET.N)
            .map(|_| (0..DATA_SET.M2).map(|_| C::BaseField::rand(rng)).collect())
            .collect();
        (Matrix::new(matrix_A1_values), Matrix::new(matrix_A2_values))
    }

    pub fn generate_circuit_with_matrices<R: ark_std::rand::Rng>(
        rc: mimc7::Parameters<C::BaseField>,
        matrix_A1: Matrix<C::BaseField>,
        matrix_A2: Matrix<C::BaseField>,
        rng: &mut R,
    ) -> Result<Self, Error> {
        let witness =
            sample_register_seeded_matprg_witness_with_matrices::<C, _>(&rc, matrix_A1, matrix_A2, rng)?;
...
fn sample_register_seeded_matprg_witness_with_matrices<C, R>(
    rc: &mimc7::Parameters<C::BaseField>,
    matrix_A1: Matrix<C::BaseField>,
    matrix_A2: Matrix<C::BaseField>,
    rng: &mut R,
    ...
    for<'a> &'a GG: GroupOpsBounds<'a, C, GG>,
{
    fn generate_constraints(
        ...
        let matrix_A1 = MatrixVar::new_constant(
            cs.clone(),
            self.matrix_A1.ok_or(SynthesisError::AssignmentMissing)?,
        )?;
        let matrix_A2 = MatrixVar::new_constant(
            cs.clone(),
    ...
    for<'a> &'a GG: GroupOpsBounds<'a, C, GG>,
{
    ...
    fn generate_circuit<R: ark_std::rand::Rng>(
        ...
        let (matrix_A1, matrix_A2) = Self::sample_public_matrices(rng);
        Self::generate_circuit_with_matrices(rc, matrix_A1, matrix_A2, rng)
    }
}
